(ns example.app
  (:require
    ...
    [example.app.system :as app.system]
    [clojure.core.server]
    [aero.core :as aero]
...
(defn -main [& args]
  (try
    (let [{config-path :config} (apply cli/init cli/default-cli-options args)
      ...
      (def system (ig/init (app.system/system-config config)))
...
;; The application will write a log file to ../example.logs/app.log.
#_;
        (example.app/-main "-c" "resources/config.edn")